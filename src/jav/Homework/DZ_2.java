...
public class DZ_2 {
    ...
    public static void methodFour(int[] myArrayFf){
        int minValue = myArrayFf[0], minIndex = 0, maxValue = myArrayFf[0], maxIndex = 0;
    ...
    public static void methodFive(int[][] myArrayG){
        for (int i = 0;i < myArrayG.length; i++){
            myArrayG[i][0] = i * 1;
            for (int j = 0, jsd = myArrayG.length; j < myArrayG.length; j++, jsd--){
                myArrayG[0][j] = j * 1;
                if (i == j || i == (jsd - 1)) myArrayG[i][j] = 1;
                System.out.print(myArrayG[i][j] + "");
            }
            System.out.print("\r\n");
        }
    }
